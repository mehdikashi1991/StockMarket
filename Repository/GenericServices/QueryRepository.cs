...
namespace Infrastructure.GenericServices
{
    public class QueryRepository<T, TInterface> : IQueryRepository<T, TInterface> where T : class, IAggegateRoot, TInterface
    {
        ...
        {
            return await _querySet.MaxAsync(selector).ConfigureAwait(false) ?? 0;
        }
        ...
        public async Task<PageResult<T>> GetPaging(int page, int pageSize, int currentPage = 1, long lastId = 0)
        {
            ...
            result.Result = await _querySet.OrderBy(i => i.Id).Where(i => i.Id > lastId).Take(pageSize).ToListAsync().ConfigureAwait(false);
